...
int main(int argc, char* argv[])
{
	...
	if (iparams.hasBlockDataFile && iparams.hasdataFile && stat(iparams.dataFile.c_str(), &info) == 0 && argc <= 2){
		...
		if (iparams.hasdataFile){
			std::cout << "Warning: The input data file \""<< iparams.dataFile << "\" from parameter file does not exist. Choose the default file \"ellipse_input.dat\"." << std::endl;
		}
	...
	*/
	...
	CGAL::create_merge_poly_files_list(full_ell);
