/*
 * cutting_off_polygons.hh
 *
 *  Created on: 16 août 2016
 *      Author: ngotr
 *
 *      Cutting off the polygons along the interface line
 */

#ifndef CUTTING_OFF_POLYGONS_HH_
#define CUTTING_OFF_POLYGONS_HH_
...
Output_Child_Polys cutting_off_polygons(CGAL::Surface_mesh< CGAL::Point_3<Kernel> > &polygon1,
	...
	output_ip = CGAL::intersections_of_two_polys<SurfaceMesh_,Output_IP> (polygon1, polygon2);
	...
	if (intersection_points_1.size() + intersection_points_2.size() < 4){
		...
		if ( (*list_IP_Ell_output.begin()).Ell_name_ == (*(boost::next(list_IP_Ell_output.begin()))).Ell_name_ ){
			...
			{
				// Remove too close points _ NTD 15/09/2016
				...
				for( ; ip_ < AddPoints2NewPoly_1.size()-1; ++ip_) {
					...
					//double distance_ =  CGAL::to_double(CGAL::squared_distance(*it_p2, *it_p1));
					double distance_ =  CGAL::sqrt( CGAL::to_double(CGAL::squared_distance(*it_p2, *it_p1)));
					int numAddedPoints = round(distance_ / target_edge_length);

					//int numAddedPoints = ceil(distance_ / target_edge_length)-1;
					//int numAddedPoints = ceil(CGAL::sqrt( CGAL::squared_distance(*it_p2, *it_p1)) / target_edge_length)-1;
					//int numAddedPoints = ceil( CGAL::div(CGAL::sqrt( CGAL::squared_distance(*it_p2, *it_p1)), target_edge_length) )-1;

					if (VERY_VERBOSE == 1){
						std::cout << "Num of added points (poly_1) = " << numAddedPoints << std::endl;
					}
					...
					if (numAddedPoints == 9 || numAddedPoints == 8 || numAddedPoints == 10){
						for (int i = 0; i!= (numAddedPoints+1); i++){
							std::cout << "Point to add 1 : " << *it_p1 + (i+1) * (*it_p2 - *it_p1)/(numAddedPoints+1) << std::endl;
						}
					}
			...
			if (DEBUG_NTD == 0){
				...
				// Child polygon poly_2_child_1
				{
					...
					for( ; ip_ < AddPoints2NewPoly_2.size()-1; ++ip_) {
						...
						//int numAddedPoints = ceil(CGAL::sqrt( CGAL::squared_distance(*it_p2, *it_p1)) / target_edge_length)-1;
						//int numAddedPoints = ceil(CGAL::div(CGAL::sqrt( CGAL::squared_distance(*it_p2, *it_p1)) / target_edge_length))-1;
						//std::cout << "Num of added points = " << numAddedPoints << std::endl;

						if (VERY_VERBOSE == 1){
							std::cout << "Num of added points (poly_2) = " << numAddedPoints << std::endl;
						}
						for (int i = 0; i!= (numAddedPoints+1); i++)
						...
						if (numAddedPoints == 9 || numAddedPoints == 8 || numAddedPoints == 10){
							for (int i = 0; i!= (numAddedPoints+1); i++){
								std::cout << "Point to add 2 : " << *it_p1 + (i+1) * (*it_p2 - *it_p1)/(numAddedPoints+1) << std::endl;
							}
						}
					}
				...
				{
					//check_conformity_poly_list(polygon2, AddPoints2NewPoly_2, list_seg_interst2);
...
#endif /* CUTTING_OFF_POLYGONS_HH_ */
