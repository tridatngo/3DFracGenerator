/*
 * exe_read_inp_write_dgf_file_log_hete_mod.cpp
 *
...
public:
	...
	T getVariance()
	{
		...
		double temp = 0;
		for(int it =0; it !=data.size(); it++)
			temp += (data[it]-mean)*(data[it]-mean);
...
int main(int argc, char** argv) {
	...
	*/
	...
	std::vector< double >				Arr_Perm_Hete(num_cells); // Array of permeability for each cell
	std::vector< double >				Arr_LogPerm_Hete(num_cells), Arr_LogSigma_Hete(num_cells);
	...
	*/
	...
	if (hete == 2){
		...
		for (int ifrac=0; ifrac < num_fracs; ifrac++)
		{
			if (Points_list_withID[ifrac].size() > 0){
				...
				outfile_Rscr << "g.dummy <- gstat(formula=z~1, locations=~x+y, dummy=T, beta=1,"
						<< "model=vgm(psill=0.025,model=\"Exp\",range=" << (*frac_radius_dum) * 0.3 << "), nmax=30)\n";
				...
				outfile_Rscr << "scale = " << log(aperture[ifrac]) << "\n";
				...
				for (int ipts = 0; ipts!= Points_list_withID[ifrac].size(); ipts++){
					...
					Aper_list_withID[ifrac].push_back(exp(db_dummy_1));
				}
	...
	for (int it=0; it != num_cells; ++it)
	{
		...
		if (hete == 1){
			...
			outfile << (*perm_i_X) * aperture[mat_ID]  << " " << (*perm_i_Y) * aperture[mat_ID] << " " << (*perm_i_Z) * aperture[mat_ID] << " ";
		...
		// Cell-to-cell heterogeneity
			...
			for (i=0; i < vertices_per_cell; i++)
				outfile << cells_[it][i] - 1 << " ";
			outfile << compute_K_from_apert_mul((arr_Apert_ID[it]).Apert_) * (arr_Apert_ID[it]).Apert_<< " ";
			outfile << compute_K_from_apert_mul((arr_Apert_ID[it]).Apert_) * (arr_Apert_ID[it]).Apert_<< " ";
			outfile << compute_K_from_apert_mul((arr_Apert_ID[it]).Apert_) * (arr_Apert_ID[it]).Apert_<< " ";
	...
	for (int it =0; it != arr_Apert_ID.size(); ++it)
	{
		if (hete == 1){
			...
			std::list<double>::iterator perm_i_X(permeability_X.begin());
			std::advance(perm_i_X, mat_ID);
			outfile_vtk << *perm_i_X * aperture[it]<< std::endl;
		}
		// Cell-to-cell heterogeneity
		else if (hete == 2){
			outfile_vtk << compute_K_from_apert_mul((arr_Apert_ID[it]).Apert_) * (arr_Apert_ID[it]).Apert_ << std::endl;
		}
	}
	outfile_vtk.close();

	// ===========================================================================
	// Calculate variance of log permeability
	// ===========================================================================

	std::string  outfile_stat_(s_cwd + "/" + filename_.substr(0,filename_.length()-4)+ "_hete.stat");
	std::string cmd("rm -rf "+ outfile_stat_);
	std::system(cmd.c_str());
	std::ofstream  outfile_stat; outfile_stat.open(outfile_stat_.c_str());

	for (int it =0; it != arr_Apert_ID.size(); ++it)
	{
		if (hete == 2) /*Cell-to-cell heterogeneity*/{
			Arr_LogPerm_Hete.push_back(log(compute_K_from_apert_mul((arr_Apert_ID[it]).Apert_)));
			Arr_LogSigma_Hete.push_back(log(compute_sigma_from_apert_mul((arr_Apert_ID[it]).Apert_)));

		}
	}
	Statistics<double> logK_stat, logSigma_stat;
	logK_stat.data = Arr_LogPerm_Hete;
	logSigma_stat.data = Arr_LogSigma_Hete;

	outfile_stat << "#=========================================================" << std::endl;
	outfile_stat << "Size of logK  = " << logK_stat.getSize() << std::endl;
	outfile_stat << std::endl << "Mean of logK  = " << logK_stat.getMean() << std::endl;
	outfile_stat << std::endl  << "Variance of logK  = " << logK_stat.getVariance() << std::endl;
	outfile_stat << std::endl  << "Standard deviation of logK  = " << logK_stat.getStdDev() << std::endl;
	outfile_stat << std::endl  << "Median of logK  = " << logK_stat.median() << std::endl;
	outfile_stat << std::endl  << "Min of logK  = " << logK_stat.Min() << std::endl;
	outfile_stat << std::endl  << "Max of logK  = " << logK_stat.Max() << std::endl;

	outfile_stat << "#=========================================================" << std::endl;
	outfile_stat << "Size of logSigma  = " << logSigma_stat.getSize() << std::endl;
	outfile_stat << std::endl << "Mean of logSigma  = " << logSigma_stat.getMean() << std::endl;
	outfile_stat << std::endl  << "Variance of logSigma  = " << logSigma_stat.getVariance() << std::endl;
	outfile_stat << std::endl  << "Standard deviation of logSigma  = " << logSigma_stat.getStdDev() << std::endl;
	outfile_stat << std::endl  << "Median of logSigma  = " << logSigma_stat.median() << std::endl;
	outfile_stat << std::endl  << "Min of logSigma  = " << logSigma_stat.Min() << std::endl;
	outfile_stat << std::endl  << "Max of logSigma  = " << logSigma_stat.Max() << std::endl;
	outfile_stat.close();
